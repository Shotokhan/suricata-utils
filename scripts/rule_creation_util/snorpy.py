...
snortForm = ("\n\n---------------------------Snort Rule Structure-------------------------------\n"
             ...
             "1. Rule Action\n"
             "2. Protocol - UDP, ICMP, TCP, IP, HTTP\n"
...
def protocol():
    ...
    while True:
        print(clear + "Please enter the desired protocol\n\n"
                      ...
                      '5. HTTP - Everybody talks about it\n'
                      'M. Main Menu\n\n\n\n\n\n\n\n\n\n')
        proto = input(': ')
        if proto == '1':
            ruleProto = 'ip ';
...
def ipv4Checker(ip):
    if re.match(r"^((?:(?:1\d\d|2[0-4]\d|25[0-5]|\d\d|\d)\.){3}(?:1\d\d|2[0-4]\d|25[0-5]|\d\d|\d)(?:/[1-3][0-9])*)$",
                ip) is not None:
        return True
    else:
        return False
...
def mandatoryChecker(var):
    if re.match(r'(\spriority:[1-5];|\sgid:\d+;|\ssid\:\d+;|\srev:\d+;|\sclasstype:.+?;|\smsg:".+?";)',
                var) is not None:
        return True
    else:
        return False
...
def gidCheck(var):
    if var == '':
        return True
    elif re.match(r'(\spriority:[1-5];|\sgid:\d+;|\ssid\:\d+;|\srev:\d+;|\sclasstype:.+?;|\smsg:".+?";)',
                  var) is not None:
        return True
    else:
        return False
...
def options():
    ...
    while True:
        ...
        msg = ' msg:"' + msg + '";'
        classtype = input(
            clear + 'Classtype - The category to be tagged when the rule is triggered\n\nEXAMPLES:\n\nATTEMPTED-USER\nWEB-APPLICATION-ATTACK\nATTEMPTED-RECON\nPOLICY-VIOLATION\nSHELLCODE-DETECT\nMISC-ATTACK\nNETWORK-SCAN\nUNKNOWN\n\n: ')
        classtype = ' classtype:' + classtype + ';'
        priority = input(
            clear + 'Priority - This sets the severity of the alert\nThis is 1-5 but typically only 1-3 are used.\n1 being severe and 3 being less severe\n\n: ')
        ...
        if check.upper() == 'N':
            continue
        elif mandatoryChecker(sid) and mandatoryChecker(rev) and mandatoryChecker(msg) and mandatoryChecker(
                classtype) and mandatoryChecker(priority) and gidCheck(gid):
            break
        else:
            continue
    while True:
        ...
        while True:
            dsize = input(
                clear + 'Data Size - As in packet data size. Ex - ">500" or "<30".\nPress enter to leave blank (optional)\n: ')
            if dsize != "":
                dsize = ' dsize:' + dsize + ';'
            reference = input(
                clear + 'Enter a URL for reference so reviewing analyst has a lead\n Press Enter to leave blank (optional)\n: ')
            if reference != '':
                reference = ' reference:url,' + reference + ';'
            ttl = input(
                clear + 'ttl - Input > or < a number to trigger on ttl. I.e. ">64" or "<2" etc...\nPress Enter to leave blank (optional)\n\n: ')
            if ttl != "":
                ttl = ' ttl:' + ttl + ';'
        ...
        while True:
            choice = input(
                clear + 'Would you like to Do A Content or Regex match on the whole payload contents?\n\'Y\'es or \'N\'o?\n\n: ')
            if choice.upper() == 'Y':
                contentRE()
                break
            elif choice.upper() == 'N':
                break
        appendList = [msg, httpVar, content, direction, icmpVar, flags, ttl, dsize, reference, classtype, priority, gid,
                      sid, rev]
        for i in appendList:
...
def contentMatch():
    ...
    while True:
        ...
        depth = ''
        options = input(
            clear + 'Would you like to input any of the following options for your content?\n\n1. Offset\n2. Depth\n3. Offset+Depth\nN. No\n: ')
        if options == '1':
            try:
                offset = int(input(
                    clear + 'Enter in the offset (bytes) into the payload to start the content match\n: '))
            except:
                continue
...
def contentRE():
    ...
    while True:
        choice = input(
            clear + 'Would you like to enter in a/another content or regex match?\n"C"ontent or "R"egex or "N"o\n: ')
        trackby = ''
        contentCount = ''
        seconds = ''
        if choice.upper() == 'C':
            tempcontent = contentMatch()
            content += tempcontent

        elif choice.upper() == 'R':
            tempcontent = ''
            regex = input(
                clear + 'Please Enter In your regex match. This will not be error checked.\nPythex.org is a good resource to build and check\n: ')
            regex = ' pcre:"/' + regex + '/is";'
            tempcontent += regex
            content += tempcontent
        elif choice.upper() == 'N':
        ...
        if occurrence.upper() == 'Y':
            ...
            while True:
                try:
                    ...
                    continue
            occurrence = ' threshold: type both , track ' + trackby + ', count ' + str(
                contentCount) + ' , seconds ' + str(seconds) + ' ;'
            break
...
def http():
    global clear
    global httpVar
    contentField = lambda field: ' http_{};'.format(field)
    reqField = lambda sticky: 'request_body' if sticky else 'client_body'
    respField = lambda sticky: 'response_body' if sticky else 'server_body'
    fields = {'1': 'method', '2': 'uri', '3': 'cookie', '4': 'user_agent', '5': reqField, '6': 'stat_msg',
              '7': respField}
    sticky, more = False, True
    while more:
        field, mode, rule = '', '', ''
        while more:
            choice = input(clear + 'Would you like to add some more rules? (Y) or (N)\n')
            if choice.upper() == 'Y':
                break
            elif choice.upper() == 'N':
                more = False
        ...
        while more:
            choice = input(
                clear + 'Do you use content modifier (C) or sticky buffer (S)? Probably content modifier if you '
                        'have Suricata up to version 4, else sticky buffer; anyway it may change according to fields\n')
            if choice.upper() == 'S':
                sticky = True
                break
            elif choice.upper() == 'C':
                ...
                print("\nError: invalid choice\n")
        while more:
            choice = input(clear + 'Would you like to do any of the following (check out '
                                   'https://suricata.readthedocs.io/en/latest/rules/http-keywords.html if you do not '
                                   'find what you need)\n '
                                   'Request fields:\n'
                                   '1. Track HTTP by Request Method http_method\n'
                                   '2. Track HTTP by Normalized Uri http_uri\n'
                                   '3. Track HTTP by Cookie http_cookie\n'
                                   '4. Track HTTP by User-Agent http_user_agent\n'
                                   '5. Track HTTP by Request Body http_client_body (or http_request_body)\n'
                                   'Response fields:\n'
                                   '6. Track HTTP by Status Message http_stat_msg\n'
                                   '7. Track HTTP by Response Body http_server_body (or http_response_body)\n'
                           )
            if choice in fields.keys():
                field = fields[choice]
                if choice in ['5', '7']:
                    field = field(sticky)
                field = contentField(field)
                break
            else:
                print("\nError: invalid choice\n")
        while more:
            choice = input(clear + 'Do you want to filter by content (C) or use a regex (R)?\n')
            if choice.upper() == 'C':
                mode = 'content'
                break
            elif choice.upper() == 'R':
        ...
        while more:
            if mode == 'content':
                ...
                regex = input(
                    clear + 'Please Enter In your regex match. This will not be error checked.\nPythex.org is a good '
                            'resource to build and check\n: ')
                rule = ' pcre:"/' + regex + '/is";'
            break
        if more:
            if sticky:
                var = field.replace("_", ".", 1) + rule
            else:
                var = rule + field
...
def tcp():
    ...
    while True:
        ...
        while True:
            choice = input(
                clear + 'TCP Flags - Would you like to match on specific flag combinations?\n\'Y\'es or \'N\'o?\n\n:')
            if choice.upper() == 'Y':
                flags = input(
                    clear + 'Enter in the first letter only once for each flag/modifier you want:\nEx - SA or SF or FSRP or +AUCFS. Modifiers must come first\n\n'
...
while True:
    ...
    workingRule = ruleAction + ruleProto + ruleSrcIP + ruleSrcPort + "->" + ruleDstIP + ruleDstPort + '(' + (
        ''.join(ruleOptions)) + ')'
    print(workingRule + '\n')
    choice = input(': ')
    if choice == '1':
        ...
        if len(ruleAction) > 0 and len(ruleProto) > 0 and len(ruleSrcIP) > 0 and len(ruleSrcPort) > 0 and len(
                ruleDstIP) > 0 and len(ruleDstPort) > 0:
            options()
        else:
            input(
                clear + 'You are not allowed to enter the options wizard until fields 1-6 are complete\nPress Enter to Return to main menu....')
    elif choice.lower() == 'r':
